<template>
  <div id="nike-like">
    <h1 class=appTtl>Nike ID Like</h1>
    <div id="wrapper">
      <svg version="1.1" id="af01" xmlns="http://www.w3.org/2000/svg" x="0" y="0" viewBox="0 0 1280 800" style="enable-background:new 0 0 1280 800" xml:space="preserve">
      <path v-bind:class="{'active': isActive === 'logo', 'focus': isActive === 'logo'}" v-on:click="activeClass('logo')" v-bind:style="{fill:elColor}"  id="logo" d="m711.2 386.6.3-.1c0 4.3-4.5 10.8-4.5 15V426c0 5.4 1.9 9.8 4.5 14.5 1.1 1.9 1.9 5.1 2.4 7.1.7 2.5 4.8 2.5 5.1 4.9.6 5.3 12.9 7.9 16.5 9.5 5.4 2.4 12.8 2.5 19 2.5h23c4.6 0 11.4-1.6 16-1 4.5.6 12.2-3.5 17-3.5 4.9 0 12.3-2 17-3 5.6-1.1 11.2-1.5 16.9-1.5 9 0 18.9-1.1 27.6-3.5 14-3.9 25.6-6.6 40-7.5 5.6-.3 18.1-3.3 23.5-4.5 5.1-1.1 10.5-1.1 15.5-3 11.9-4.5 19.5-4 31.5-8 4.4-1.5 10.2-1.9 14.5-3.5 2.7-1 6.2-2.8 9-3 5.3-.4 6.5.2 11.5-1.5 3.4-1.2 8.7-.7 12-2.5 4.8-2.6 8.2-2.2 13.5-2.5 2.7-.2 8.7 1.3 7.5-3-.8-2.7-11.4-5.5-14-6.5-4.8-1.8-13.5-5.3-17.5-8.5-4.9-3.9 11.9-8.3 14.5-8.5 11-5 4.8-5.5 15-11.5 4.4-2.6 18.7-23.1 22.5-26 5.6-4.2 5.2-11.2 10-16 4.4-4.4 11.4-15 14.5-20.5 2.1-3.8 8.1-9 13-9 13.5 0 17.1 14.4 17.5 26 5.5 12.7 4.7 23.9 8 34.5 3.8 12.5 7 27.2 7 40.5 0 6.8 3.5 13.4 5 20 1.1 4.8 2.2 9.7 2.5 14.7.3 4.4 1 7.2 1 11.8v.1c0 2.4 0 4.7 1.5 6.7.7.9-2.6 2.9-5.8 0-7.2-6.5-4.6-1.9-11.7-8.3-1.8-1.7-11.2-12.2-13-14-2-2-6.9-7.5-9.5-9-4.7-2.7-9.1 1.8-15 3.5-6.2 1.8-14.4 2.8-20.5 5.5-10 4.4-20 5-30 10-8.6 4.4-17.7 4.7-26.5 8.5-4.2 1.8-11.6 3.4-16.1 4.4l-.4.1c-5.2 1.1-11.8 4.8-17 5-5.6.2-8.5 4-14 5-8.6 1.6-15.7 5-24.5 5.4-9.9.5-19.1 7.6-29 7.6-3.8 0-9.9 3.1-13.5 4-10.6 2.6-18.4 5.3-29.5 6-10.6.6-20.2 5.6-30.5 7.5-4.4.8-10.7 3-15 4-8 1.8-14.7 4.5-23 4.5-6.3 0-12.6 1-18.5 3.3-9.2 3.6-17.4 3-27 5.2-4.8 1.1-8.2 1.6-13 3-4.9 1.5-10 3.5-15 4-8.8.9-17.1 2.5-26 2.5-7.6 0-18.9-2.5-26.5-2.5-6.9 0-12.7-1.2-19-4-4.2-1.8-8.9-2.1-11.5-6-1.7-2.5-7.1-5.6-9.5-8-2.2-2.2-3.4-5.7-5-8.5-2.6-4.7-11-17.5-8.5-23-1-5.5.6-12.8 2-19 1.6-6.8 4.1-10.1 8.1-16 1.9-2.9 2.6-7.1 4.9-9.5 1.8-1.8 5.7-4.5 7.8-5.9l.2-.1c4.5-3 5.9-7 9.5-11 1.8-2 5.9-4.9 8.5-6 4.5-2 7.4-7.2 11.5-9.5s8.4-3.2 12-6c4.8-3.7 12.8-5.5 19.7-6.9z" />
      <path v-bind:class="{'active': isActive === 'sole', 'focus': isActive === 'sole'}" v-on:click="activeClass('sole')" v-bind:style="{fill:elColor_sole}" id="sole" d="M121 672.5c-6.4-12.1-2.5-21-2.5-37 7 0 9 4.5 16 4.5 6.4 0 13.2 4.5 18.5 4.5 13.2 0 18.5 14.1 31 18 9.5 3 14.2 10.7 23.6 13.9 10 3.4 62.3 12.4 71.9 17.1 13.5 0 12.5 0 42 3 23.5 0 96 5 119.5 5 19-1.2 38.2.1 57 2 38.7 3.9 76.7 1.8 114.5 4 52.6 3.1 106.7.5 159.5.5 51.5-2.5 103.8-2.5 155.5-2.5 18.3 0 37.7-2 56-2 20.7 0 43.3 3.6 63-2 10.4-3 21.2.5 32.5.5 14.8 0 26.1-2.8 40-5.9 6.4-1.4 32.8-11.4 38.6-5.7 1.7 1.7-5.3 15-6.1 17.6-2.2 7.1-10.1 16-18 16-18.9 0-24 7.5-54.5 5-13 .5-13.4 2.9-21.5 2.9-12.8 0-24.6-2.7-36.5 0-7.6 1.7-14.3 1.6-22 1.6-12 0-27 2.1-38.6 2.6-8.8.3-17.7 0-26.4 0H796c-34.4 0-67.7 2-102.1 2-57.5 0-115.8-4.5-173.4-4.5H426c-37.3 0-67-6-104.5-6-25.1 0-60-5.6-71.5-9.5-14.2-4.8-32.9-1.8-46-8-8.7-4.1-17.7-2.9-27-5.5-10.5-3-10.5-2.5-33.5-6.5-13.5-3.1-19.9-20.8-22.5-25.6z" style="fill:#ff0606"/></svg>
      <!-- <svg id="af01" viewBox="0 0 1280 1280" xmlns="http://www.w3.org/2000/svg">
        <path v-bind:class="{'active': isActive === 'logo', 'focus': isActive === 'logo'}" v-on:click="activeClass('logo')" v-bind:style="{fill:elColor}"  id="logo" d="m711.222 626.56.278-.06c0 4.279-4.5 10.801-4.5 15V666c0 5.404 1.896 9.812 4.5 14.5 1.079 1.943 1.883 5.09 2.444 7.056.713 2.495 4.789 2.539 5.056 4.944.588 5.291 12.939 7.917 16.5 9.5 5.436 2.416 12.807 2.5 19 2.5h23c4.622 0 11.447-1.569 16-1 4.468.558 12.18-3.5 17-3.5 4.892 0 12.3-2.039 17-3 5.597-1.145 11.237-1.5 16.944-1.5 9.026 0 18.889-1.109 27.556-3.5 13.965-3.852 25.578-6.626 40-7.5 5.565-.337 18.069-3.319 23.5-4.5 5.114-1.112 10.55-1.13 15.5-3 11.922-4.504 19.458-3.986 31.5-8 4.36-1.453 10.174-1.927 14.5-3.5 2.726-.991 6.22-2.794 9-3 5.31-.394 6.54.217 11.5-1.5 3.4-1.177 8.65-.699 12-2.5 4.76-2.561 8.19-2.188 13.5-2.5 2.72-.16 8.74 1.325 7.5-3-.78-2.72-11.4-5.502-14-6.5-4.79-1.841-13.48-5.286-17.5-8.5-4.88-3.903 11.89-8.347 14.5-8.5 11-5 4.83-5.52 15-11.5 4.45-2.616 18.69-23.14 22.5-26 5.59-4.191 5.22-11.217 10-16 4.37-4.366 11.45-15.009 14.5-20.5 2.09-3.769 8.13-9 13-9 13.51 0 17.05 14.371 17.5 26 5.5 12.722 4.74 23.898 8 34.5 3.84 12.485 7 27.164 7 40.5 0 6.813 3.54 13.422 5 20 1.06 4.759 2.2 9.657 2.5 14.722.26 4.364 1 7.159 1 11.778v.124c0 2.405 0 4.724 1.5 6.654.68.877-2.61 2.851-5.78 0-7.2-6.485-4.59-1.865-11.72-8.278-1.84-1.66-11.16-12.159-13-14-2-2.003-6.95-7.543-9.5-9-4.7-2.684-9.11 1.755-15 3.5-6.23 1.845-14.38 2.781-20.5 5.5-9.99 4.439-20.04 4.964-30 10-8.62 4.362-17.7 4.652-26.5 8.5-4.16 1.819-11.58 3.432-16.13 4.42l-.37.08c-5.22 1.135-11.85 4.802-17 5-5.577.215-8.515 3.992-14 5-8.614 1.582-15.704 4.956-24.5 5.444-9.853.548-19.114 7.556-29 7.556-3.78 0-9.896 3.099-13.5 4-10.58 2.645-18.393 5.327-29.5 6-10.582.641-20.197 5.627-30.5 7.5-4.406.801-10.668 3.037-15 4-8.032 1.785-14.746 4.5-23 4.5-6.299 0-12.589.965-18.5 3.278-9.235 3.614-17.432 2.971-27 5.222-4.847 1.14-8.209 1.58-13 3-4.925 1.459-9.961 3.47-15 4-8.819.928-17.147 2.5-26 2.5-7.593 0-18.907-2.5-26.5-2.5-6.869 0-12.694-1.197-19-4-4.157-1.847-8.896-2.093-11.5-6-1.691-2.537-7.065-5.565-9.5-8-2.237-2.237-3.436-5.685-5-8.5-2.627-4.728-11.008-17.482-8.5-23-1-5.5.592-12.839 2-19 1.562-6.836 4.094-10.057 8.056-16 1.949-2.925 2.586-7.142 4.944-9.5 1.753-1.753 5.73-4.457 7.85-5.898L650 666c4.474-3.043 5.947-7.02 9.5-11 1.774-1.987 5.92-4.853 8.5-6 4.487-1.994 7.411-7.228 11.5-9.5 4.135-2.297 8.425-3.219 12-6 4.819-3.748 12.786-5.455 19.722-6.94Z" fill="#FF0D0D"/>
        <path v-bind:class="{'active': isActive === 'sole', 'focus': isActive === 'sole'}" v-on:click="activeClass('sole')" v-bind:style="{fill:elColor_sole}" id="sole" d="M121 912.5c-6.404-12.096-2.5-21-2.5-37 7 0 9 4.5 16 4.5 6.368 0 13.216 4.5 18.5 4.5 13.228 0 18.534 14.104 31 18 9.487 2.965 14.247 10.722 23.556 13.944 9.962 3.449 62.31 12.377 71.944 17.056 13.5 0 12.5 0 42 3 23.519 0 95.981 5 119.5 5 18.964-1.223 38.178.118 57 2 38.654 3.865 76.658 1.774 114.5 4 52.639 3.096 106.727.5 159.5.5 51.5-2.5 103.833-2.5 155.5-2.5 18.333 0 37.667-2 56-2 20.73 0 43.32 3.624 63-2 10.4-2.972 21.24.5 32.5.5 14.82 0 26.07-2.792 40-5.889 6.38-1.417 32.84-11.379 38.56-5.667 1.73 1.73-5.27 14.983-6.06 17.556-2.19 7.114-10.13 16-18 16-18.93 0-24 7.5-54.5 5-13 .5-13.44 2.889-21.5 2.889-12.83 0-24.55-2.655-36.5 0-7.57 1.682-14.3 1.555-22 1.555-11.952 0-26.95 2.126-38.556 2.556-8.783.325-17.651 0-26.444 0H796c-34.37 0-67.741 2-102.111 2-57.465 0-115.833-4.5-173.389-4.5H426c-37.274 0-67.045-6-104.5-6-25.136 0-60-5.598-71.5-9.5-14.19-4.815-32.878-1.785-46-8-8.714-4.127-17.746-2.856-27-5.5-10.549-3.014-10.5-2.5-33.5-6.5-13.5-3-19.944-20.671-22.5-25.5Z" fill="#FF0606"/>
      </svg> -->
      <img id="bg_img" src="@/assets/images/af1_01_v02.jpg" alt="Air Force1">
      <input type="color">
    </div>
    <div class="ui_wrapper">
      <ul class="colors">
        <li v-on:click="drawColor(color)" v-for="color in colors" v-bind:key="color" v-bind:style="{'background-color': color}" class="color"> </li>
      </ul>
      <ul class="operation">
        <li><button id="save" v-on:click="dataSave">Save</button></li>
      </ul>
    </div>
    <ul id="collection">

      <li v-for="item in items" v-bind:key="item">{{item.logo}}{{item.sole}}</li>
    </ul>
  </div>
</template>

<script>
import axios from 'axios';
export default {
  data: function() {
    return {
      isActive: '',
      isFocus: '',
      colors: [
        '#8cd147',
        '#4a9ccf',
        '#661f45',
        '#1e2024'
      ],
      elColor: '',
      elColor_sole: '',
      items: []S
    }
  },
  created: function() {
    axios.get(
      'https://firestore.googleapis.com/v1/projects/t-nike-id/databases/(default)/documents/shoes-color',
    )
    .then(response => {
      items = response.data.documents
    })
    .catch(error => {
      console.log(error)
    })
  },

  methods: {
    activeClass: function(el) {
      this.isActive = el;
    },
    drawColor: function(color) {
      let hex = color;
      if(this.isActive === 'logo') {
        this.elColor = hex;
      } else if(this.isActive === 'sole') {
        this.elColor_sole = hex;
      }
    },
    dataSave: function() {
      // サーバーに情報を入れる
      // axios.post(第一引数(URL),第二引数(データ),第三引数);
      axios.post(
        'https://firestore.googleapis.com/v1/projects/t-nike-id/databases/(default)/documents/shoes-color',
        {
          fields: {
            logoData: {
              stringValue: this.elColor
            },
            soleData: {
              stringValue: this.elColor_sole
            },
          }
        }
      )
      .then(response => {
        console.log(response);
      }) 
      .catch(error => {
        console.log(error)
      })
      // -----------------------------------------------------------
    }
  }
}
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped>
#nike-like {
    background-color: #F5F5F5;
}

.appTtl {
  position: absolute;
  top: 20px;
  z-index: 100;
  color: #666;
  margin: 0;
  font-size: 30px;
  left: 20px;
}
#wrapper {
  position: relative;
  width: 100vw;
  max-width: 768px;
  margin: 0 auto;
  height: 100vh;
  background-color: #F5F5F5;
}

/* SVG本体 */
#af01 {
  position: relative;
  top: 20%;
  z-index: 10;
  mix-blend-mode: screen;
}

#af01 > path.focus {
  stroke: #ccc;
  stroke-width: 4px;
  stroke-dasharray: 15 5;
}

/* 背景画像 */
#bg_img {
  position: absolute;
  top: 20%;
  left: 0;
  width: 100%;
  height: auto;
}

@media screen and (orientation: landscape) and (min-width: 556px) and (max-width: 768px) {
  #wrapper {
    max-width: 460px;
  }
  
}
@media screen and (min-width: 556px) {
  #af01, #bg_img {
    top: 0;
  }
}
.ui_wrapper {
  position: fixed;
  bottom: 2em;
  right: 2em;
  z-index: 1000;
  display: flex;
  justify-content: space-between;
  align-items: center;
}

.colors {
  display: flex;
  list-style: none;
}

.color {
  width: 50px;
  height: 50px;
  margin-left: 25px;
  border-radius: 50%;
  border: 2px solid #aaa;
  background-color: pink;
  cursor: pointer;
}

/* 操作ボタン */
.operation {
  list-style: none;
  display: flex;
  align-items: center;
  margin-left: 100px;
}

.operation button {
  background-color: #ccc;
  border: solid 1px #999;
  border-radius: 999999px;
  padding: 0.2em 0.5em;
}
</style>
